...
def main(args):
    ...
    dm = PerturbationDataModule(
        datasplits=datasets, train_bs=args["model"]["hparams"]["batch_size"]
    )
    dataset_config = {
    ...
    print(
        f"drugs_idx range: {drugs_idx.min().item()} to {drugs_idx.max().item()}, total num_drugs={dataset_config['num_drugs']}"
    )
    ...
    if args["model"]["load_pretrained"]:
        ...
        if model_hash and checkpoint_path.exists():
            ...
            for key in keys:
                if (
                    key.startswith("model.adversary_drugs")
                    or key.startswith("model.drug_embeddings")
                    or key.startswith("drug_embeddings")
                    or key.startswith("adversary_drugs")
                ):
                    state_dict.pop(key)
            ...
            if args["model"]["embedding"]["model"] == "vanilla":
                ...
                for key in keys:
                    if (
                        key.startswith("model.dosers")
                        or key.startswith("model.drug_embedding_encoder")
                        or key.startswith("dosers")
                        or key.startswith("drug_embedding_encoder")
                    ):
                        state_dict.pop(key)

            # Handle covariate embeddings - map between old and new indices
            if "model.covariates_embeddings.0.weight" in state_dict:
                old_embeddings = state_dict[
                    "model.covariates_embeddings.0.weight"
                ]  # e.g. shape [82, 32]
                embedding_dim = old_embeddings.shape[1]

                new_cell_types = np.unique(
                    datasets["training"].covariate_names["cell_type"]
                )
                ...
                new_embeddings = torch.zeros(
                    (num_covariates, embedding_dim), device=old_embeddings.device
                )
                ...
                for new_idx, cell_type in enumerate(new_cell_types):
                    if cell_type in old_cell_types:
                        ...
                        print(
                            f"Warning: Cell type {cell_type} not found in pretrained model"
                        )
            ...
            if "model.adversary_covariates.0.network.9.weight" in state_dict:
                old_layer_weight = state_dict[
                    "model.adversary_covariates.0.network.9.weight"
                ]
                old_layer_bias = state_dict[
                    "model.adversary_covariates.0.network.9.bias"
                ]
                ...
                new_layer_weight = torch.zeros(
                    (num_covariates, hidden_dim), device=old_layer_weight.device
                )
                new_layer_bias = torch.zeros(
                    num_covariates, device=old_layer_bias.device
                )
                ...
                state_dict["model.adversary_covariates.0.network.9.weight"] = (
                    new_layer_weight
                )
                state_dict["model.adversary_covariates.0.network.9.bias"] = (
                    new_layer_bias
                )
        ...
        print("Warning: No pretrained hash found for model")

    wandb_logger = WandbLogger(**args["wandb"], save_dir=WB_DIR)
    run_id = wandb_logger.experiment.id

    checkpoint_callback = ModelCheckpoint(
        dirpath=Path(args["training"]["save_dir"]) / run_id, save_last=True
    )
